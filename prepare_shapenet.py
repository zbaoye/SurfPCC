...
def convert_to_mesh(instance_dir, sample_strategy, simplify=False, output_dir='./data/shapenet/mesh'):
    ...
    for instance in instance_dir:
        ...
        instance_path = os.path.join(instance, 'models/model_normalized.obj')
...
def generate_path_txt(pcd_dir, output_dir):
    ...
    train_path = montecarlo_sample_train_path
    test_path = montecarlo_sample_test_path
    val_path = montecarlo_sample_val_path
    # generate txt file
...
def divide_cube(xyzs, attribute, map_size=100, cube_size=10, min_num=100, max_num=30000, cur_sample_strategy=None):
    ...
    for tuple_cube_idx, point_idx in cubes.items():
        ...
        # savemat('test'+str(tuple_cube_idx)+'.mat',{'point_cloud':map_xyzs[point_idx, :]})
...
def generate_dataset(mode='train', dataset_name='shapenet', cube_size=20, min_num=100, max_num=100000, save_path='./data/shapenet'):
    ...
    for item in pcd_item:
        ...
        mask, points, meta_data = divide_cube(xyzs, attribute=normals, cube_size=cube_size, min_num=min_num, max_num=max_num, cur_sample_strategy=cur_sample_strategy)
        key = Counter(mask)
...
def parse_dataset_args():
    ...
    # data root
    parser.add_argument('--data_root', default='/home/zby/datasets', type=str, help='dir of shapenet core dataset')
    ...
    parser.add_argument('--split_rate', default=1, type=float, help='split rate for montecarlo sampling and subdivision')
    ...
    parser.add_argument('--test_min_num', default=1024, type=int, help='minimum points number in each cube when testing')
...
if __name__ == '__main__':
    ...
    dataset_args.data_root = os.path.join(dataset_args.data_root, 'ShapeNetCore.v2')
    ...
    generate_path_txt(dataset_args.output_pcd_dir, './data/shapenet')
